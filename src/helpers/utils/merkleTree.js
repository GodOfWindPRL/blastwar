...
export function genRootHash(whiteList) {
...
export function genLeaveHashes(chunks) {
  const leaves = [];
  chunks.forEach((data) => {
...
export function createParent(leftChild, rightChild) {
  const hash =
...
export function buildHash(...data) {
  return web3.utils.soliditySha3(...data);
}

export function getMerklePath(data) {
  const hash = buildHash(data);
...
export function generateMerklePath(node, path = []) {
...
export function verifyPath(data, path) {
